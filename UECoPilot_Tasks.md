...
- [ ] Run PIE in `D:\omoba\omfue\Content\Map\Main.umap` and confirm first viewport is playable, not only loadable:
...
- [ ] Implement or fix `BP_PlayerController` input graph so it matches this priority order:
  - UI capture first.
...
- [ ] Implement PIE interaction path from UI to Rust bridge:
...
- [ ] Make `WBP_OmHudRoot` a real playable HUD:
...
- [ ] Make tower placement visibly usable:
...
- [ ] Make selected tower actions visibly usable:
...
- [ ] Verify `BP_RtsCameraPawn` controls in real PIE input:
...
- [ ] Verify `BP_MapRoute` visual output:
...
- [ ] Complete Saika hero visuals in `/Game/RustBP/Heroes/BP_SaikaMagoichi`:
...
- [ ] Complete Saika ability and buff visuals:
...
- [ ] Complete tower/projectile visuals:
...
- [ ] Add UECoPilot PIE screenshot or log evidence to `D:\omoba\omfue\README.md` after verification:

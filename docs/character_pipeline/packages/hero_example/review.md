...
`hero_example` 是遠程 marksman，核心辨識是深紅長外套、compact arc rifle、cyan crystal glow。
...
- `omfx/data/heroes/hero_example/hero_example.glb` 是未來 import slot，第一版不會建立該檔。
- `hero_example_e` mobility 行為可能需要 script/runtime 支援，正式實作前需確認 movement effect API。

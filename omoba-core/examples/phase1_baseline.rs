...
fn main() {
    let mut entities = vec![(0i64, 0i64); 10_000];
    let mut samples = Vec::with_capacity(1200);
    let start = Instant::now();
    for tick in 0..1200u64 {
        let at = Instant::now();
        for (index, (x, y)) in entities.iter_mut().enumerate() {
            *x = x.wrapping_add((index as i64 + tick as i64) & 3);
            *y = y.wrapping_sub((index as i64 ^ tick as i64) & 3);
        }
        samples.push(at.elapsed().as_micros() as u64);
    }
    let payload = TickBatch {
        tick: 1,
        inputs: vec![],
        server_events: vec![],
    ...
    .encode_to_vec();
    let wire_bytes = payload.len() + 5;
    let bandwidth = wire_bytes as f64 * 120.0;

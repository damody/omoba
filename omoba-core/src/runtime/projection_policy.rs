...
pub const REQUIRED_PROJECTION_POLICIES: &[(&str, &str)] = &[
    (projection_policy_ids::MOVEMENT, "runtime/native/tick/movement"),
    (projection_policy_ids::SPAWN, "runtime/native/game_processor/spawn"),
    (projection_policy_ids::DEATH, "runtime/native/tick/death"),
    (projection_policy_ids::OWNERSHIP, "runtime/native/game_processor/ownership"),
    (projection_policy_ids::DIRECT_COMBAT, "runtime/native/tick/damage"),
    (projection_policy_ids::PROJECTILE, "runtime/native/tick/projectile"),
    (projection_policy_ids::AOE, "runtime/native/game_processor/aoe"),
    (projection_policy_ids::BUFF_DEBUFF, "runtime/native/tick/buff"),
    (projection_policy_ids::HERO_ABILITY, "runtime/native/tick/hero"),
...
impl ProjectionPolicyRegistry {
    ...
    pub fn register_str(&mut self, action_id: &str, source_module_path: &str) {
        self.registrations.insert(action_id.to_owned(), ProjectionPolicyRegistration {
    ...
    pub fn register_script_policy(
        ...
        let known: BTreeSet<_> = REQUIRED_PROJECTION_POLICIES.iter().map(|(id, _)| *id).collect();
...
pub fn validate_secure_match_startup(
    registry: &ProjectionPolicyRegistry,
) -> Result<(), String> {
    registry.validate_complete().map_err(|missing| {
        let entries = missing.into_iter().map(|entry| {
            format!("{} ({})", entry.action_id, entry.source_module_path)
        }).collect::<Vec<_>>().join(", ");

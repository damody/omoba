...
impl Unit {
    ...
    /// 從戰役 creep 資料創建單位
    pub fn from_creep_data(creep_data: &crate::runtime::scene::import_campaign::CreepJD) -> Self {
    ...
    pub fn from_enemy_data(enemy_data: &crate::runtime::scene::import_campaign::EnemyJD) -> Self {
        use omoba_template_ids::{active_creep_display, active_creep_stats, creep_by_name};
    ...
    /// 創建雜賀鐵炮兵召喚單位
    pub fn create_saika_gunner(position: (f32, f32), _owner_team: i32) -> Self {
        let mut unit = Unit {
    ...
    /// 創建弓箭手召喚單位
    pub fn create_archer(position: (f32, f32), _owner_team: i32) -> Self {
        let mut unit = Unit {
            id: "archer".to_string(),
            name: "弓箭手".to_string(),
            unit_type: UnitType::Summon,
    ...
    /// 創建劍士召喚單位
    pub fn create_swordsman(position: (f32, f32), _owner_team: i32) -> Self {
        let mut unit = Unit {
    ...
    /// 創建法師召喚單位
    pub fn create_mage(position: (f32, f32), _owner_team: i32) -> Self {
        let mut unit = Unit {
            id: "mage".to_string(),
            name: "法師".to_string(),
            unit_type: UnitType::Summon,

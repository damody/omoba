...
fn projection_policy_catalogue_is_complete_and_unknown_ids_fail_closed() {
    ...
    assert_eq!(
        empty.validate_complete().unwrap_err().len(),
        REQUIRED_PROJECTION_POLICIES.len()
    );
}
...
fn team_identity_is_scoped_monotonic_and_never_reuses_retired_ids() {
    ...
    assert!(matches!(
        a.canonical_for(first.replica_id, first.disclosure_epoch),
        Err(TeamIdentityError::StaleDisclosureEpoch)
    ));
    let retired = a.forget(key).unwrap();
    let next = a
        .disclose(CanonicalEntityKey {
            id: 5,
            generation: 2,
        })
        .unwrap();
...
fn bounded_random_tape_accepts_only_current_disclosed_epoch_and_sufficient_values() {
    ...
    assert_eq!(
        replica.apply_frame(malformed, &mut stepper),
        Err(ReplicaRuntimeError::MalformedRandomTape)
    );
}
...
fn canonical_merge_is_independent_of_shard_completion_order() {
    let make = |ordinal| OrderedFact {
        key: FactOrderingKey {
            ...
            canonical_source_order: ordinal,
            local_ordinal: 0,
    ...
    let order: Vec<_> = buffer
        ...
        .unwrap()
        .into_iter()
        .map(|f| f.key.canonical_source_order)
        .collect();
...
fn malformed_transition_and_rebase_are_rejected() {
    ...
    let mut stepper = NoopDisclosedWorldStepper;
    let mut reveal =
        TeamTickFrame::decode(single_reveal_frame_fixture(TEAM, 0, 0, 1, COMPONENT).as_slice())
            .unwrap();
    if let Some(crate::game_proto::transition::Transition::Reveal(value)) =
        reveal.pre_step.as_mut().unwrap().transitions[0]
            .transition
            .as_mut()
    {
    ...
    assert_eq!(
        replica.apply_frame(reveal, &mut stepper),
        Err(ReplicaRuntimeError::MalformedTransition)
    );
    ...
    assert_eq!(
        replica.apply_verified_rebase(&fixture.filtered_snapshot, &fixture.manifest, &disclosed),
        Err(ReplicaRuntimeError::UnverifiedRebase)
    );
}
...
fn remembered_presentation_is_excluded_from_simulation_and_hash() {
    ...
    empty
        .apply_frame(empty_frame(0, 0, 1), &mut stepper)
        .unwrap();
    empty
        .apply_frame(empty_frame(1, 1, 2), &mut stepper)
        .unwrap();
    assert_eq!(
        with_memory.canonical_team_hash(),
        empty.canonical_team_hash()
    );

...
- [ ] 6.6 grep guard — `input_id` 不出現在 sim 路徑：grep `input_id` 在 `omb/src/comp/`、`omb/src/tick/`、`omoba-sim/src/` — 預期 0 命中（除 `comp/lockstep_resources.rs` 等 wire-edge metadata 模組）

...
目前 lockstep transport 已能替不同前端分配 `player_id`，但 gameplay ownership 仍多處等同於「第一個 Player 英雄」或「任一 Player 塔」。這會讓第二個前端即使成功連線，也無法可靠控制自己的英雄，且可能操作到其他玩家的塔。
...
- 兩個前端 SHALL 使用不同 `OMB_PLAYER_NAME` / `OMB_LOCKSTEP_PLAYER_NAME`，並連到同一個 `127.0.0.1:50061` 後端。
...
- 影響 omfx 啟動設定與環境變數讀取，包含 `OMB_PLAYER_NAME`、`OMB_LOCKSTEP_PLAYER_NAME`、log/window 識別，以及同機雙前端同時執行時的輸出檔衝突處理。
...
- 影響 omb lockstep join/session 行為與測試，需驗證兩個 player 都取得穩定 `player_id` 並可同時送 input。

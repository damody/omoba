...
目前 omb 的 KCP lockstep join path 會在 `JoinRequest` 時分配遞增 `player_id`，`TickBatch` 也會保留 `(player_id, PlayerInput)`。omfx 也已支援用環境變數設定 `OMB_PLAYER_NAME` 與 `OMB_LOCKSTEP_PLAYER_NAME`，並把 input 送到 lockstep client。
...
- 兩個前端使用不同 player name / lockstep player name，並共用同一個 server address、story、script DLL。
...
### Decision: 新增獨立 owner 資料，不重載 `Faction.team_id`

新增或復用一個清楚表示 lockstep owner 的 component/resource，例如 `PlayerOwner { player_id: u32 }`，套用在 Hero 與玩家建造的 Tower。`Faction` 繼續表示 combat side，避免 tower/hero attack filtering、vision、creep targeting 因為 `team_id` 改成 player id 而產生副作用。

替代方案是把 `Faction.team_id` 改成 `player_id`。這看似省 component，但會把「玩家身份」與「戰鬥隊伍」綁死；目前 Player vs Enemy 規則已有 team 0/1 約定，改動風險較高。

### Decision: join 分配與 hero 綁定保持 deterministic

`player_id` 仍由 omb lockstep state 分配。runtime initialization 需在 authoritative world 與 local replica world 建立相同的 player hero owner mapping。若 TD_1 story 只有一個 hero template，第二個 player 可複製同一 hero template 並放在不同 spawn offset；若 story 已提供多個 heroes，則依穩定順序綁定 player 1、player 2。
...
- [Risk] 使用 `Faction.team_id` 作 owner 可能破壞戰鬥規則。→ Mitigation：ownership 使用獨立 component/resource，`Faction` 維持 combat side。
- [Risk] `run_2player.bat` 啟動兩個 GUI process 後 cleanup 不完整。→ Mitigation：記錄 backend PID，前端 wait 後停止後端；啟動前沿用 stale `taskkill` 清理。
- [Risk] 第二個 hero 使用同 template 可能影響辨識。→ Mitigation：至少透過 name/log/player_id/position offset 區分；角色選擇與不同英雄配置留給後續 change。

## Migration Plan

1. 新增 ownership component/resource 並註冊到 authoritative 與 local replica world。
2. 調整 hero initialization，確保 TD_1 至少產生兩個 `PlayerOwner` heroes。
3. 調整 shared `GameProcessor` owner lookup，讓所有 player-owned input 使用 `player_id`。
4. 調整 tower spawn/sell/upgrade，tower spawn 寫入 owner，sell/upgrade 驗證 owner。
5. 調整 snapshot/render metadata，讓 omfx 能知道 tower owner 與 local player id。
6. 新增 `run_2player.bat` 與雙前端 log/window identity。
7. 加入 unit/integration tests，最後用 `run_2player.bat` 做人工 smoke。

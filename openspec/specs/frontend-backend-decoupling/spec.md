...
定義 `omfx` native frontend 與 `omb` backend 的依賴邊界，確保 frontend 不再直接依賴或啟動 backend crate/process，並透過 `omoba-core::runtime` 與 shared protocol 維持 lockstep simulation 行為一致。
